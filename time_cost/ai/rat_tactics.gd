...
static func choose(game: RefCounted, actor_id: StringName = &"rat", target_id: StringName = &"player") -> TacticalChoice:
	...
	var in_melee_range: bool = game.can_melee_reach(game.get_actor_position(actor_id), game.get_actor_position(target_id))
	var aggression: int = game.get_actor(actor_id).aggression
	var fear: int = game.get_actor(actor_id).fear()
	...
	if not in_melee_range:
		...
		if next_step == game.get_actor_position(actor_id):
			...
			var advance := TacticalChoice.new(MoveAction.new(next_step - game.get_actor_position(actor_id)), &"approach_target", 70)
		...
		var bite := TacticalChoice.new(AttackAction.new(target_id), &"fight_target", 100)
	...
	if fear > 0:
		var direction := _best_retreat_direction(game, actor_id, target_id)
...
static func _best_retreat_direction(game: RefCounted, actor_id: StringName, target_id: StringName) -> Vector2i:
	var best_direction := Vector2i.ZERO
	var offset: Vector2i = game.get_actor_position(actor_id) - game.get_actor_position(target_id)
	...
	for direction: Vector2i in game.MOVE_DIRECTIONS:
		...
		if not game.can_step(game.get_actor_position(actor_id), direction) or game.blocks_actor_movement(target, actor_id):
			continue
		var candidate_distance: int = (target - game.get_actor_position(target_id)).length_squared()
