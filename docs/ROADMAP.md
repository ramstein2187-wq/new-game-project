...
| World generation | Layered global geography prototype with variable world size, deterministic terrain/climate/hydrology, and separate landform/biome data | Complete on task branch | [M020](milestones/M020_world_generation_v2.md), `chat/worldgen-v2-prototype`; automated validation complete. Separate global layer; region/local-map integration remains a follow-up. |
| World map viewer | Standalone global debug scene with 10 layers, seed/size input, river overlay, zoom and hover values | Complete on task branch; manual visual review pending | [M021](milestones/M021_world_map_viewer.md), `chat/worldgen-v2-viewer`, based on M020; does not replace F5 playable scene. |
